...
- [ ] 在目标 WSL2 机器拉取、安装并运行测试。
- [ ] 指向本地 Qwen2.5 模型并保存环境检查结果。
...
## 4. 下一阶段：P1 标准与 Evaluation Dataset V1

### 工作包

1. 将 Pydantic 契约导出为版本化 JSON Schema。
2. 编写标注规范，明确主类别选择、三态决策和歧义处理。
3. 设计 100 条样本清单：Shell 30、PowerShell 20、CMD 10、Python 30、混合脚本 10。
4. 每种语言同时覆盖危险、正常、边界和注入样本。
5. 实现 JSONL 校验器和数据统计脚本。
6. 人工逐条复核标签与摘要，冻结 `eval-v1`。
