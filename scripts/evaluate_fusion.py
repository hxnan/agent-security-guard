...
DEFAULT_OUTPUT = REPOSITORY_ROOT / "artifacts" / "fusion-eval-v1" / "report.json"
...
def _compact_summary(report: dict[str, object], output: Path) -> dict[str, object]:
    repair = report["model_repair_metrics"]
    ...
    return {
        ...
        "benign_false_positive_count": report["benign_false_positive_count"],
        "high_risk_allow_miss_count": report["high_risk_allow_miss_count"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        write_fusion_evaluation_report(args.output, report)
    except OSError as exc:
        return _error("report_write", f"could not write fusion report: {exc}", 1)
